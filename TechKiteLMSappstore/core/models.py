...
from TechKiteLMSappstore.core.facades import distinct
from TechKiteLMSappstore.core.rating import compute_rating
from TechKiteLMSappstore.core.versioning import pad_min_version, \
...
class App(TranslatableModel):
    ...
    def _get_grouped_releases(self, get_release_func):
        releases = TechKiteLMSRelease.objects.all()
...
class TechKiteLMSReleaseManager(Manager):
    def get_current(self):
        return self.get_queryset().filter(is_current=True)[:1]


class TechKiteLMSRelease(Model):
    objects = TechKiteLMSReleaseManager()
    version = CharField(max_length=100, verbose_name=_('TechKiteLMS version'),
    ...
    has_release = BooleanField(verbose_name=_('Has a release'),
                               help_text=_(
                                   ...
                                   'TechKiteLMS version that can be '
    ...
    class Meta:
        verbose_name = _('TechKiteLMS release')
        verbose_name_plural = _('TechKiteLMS releases')
        ordering = ('-version',)

    @staticmethod
    def get_current_main():
        current = TechKiteLMSRelease.objects.get_current()
        if len(current) > 0:
