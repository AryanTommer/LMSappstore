...
Then clone the repository and build the image::
    ...
    git clone https://github.com/TechKiteLMS/appstore
    cd appstore
...
Export your container to be able to upload it to your production server::
    ...
    sudo docker save -o TechKiteLMSappstore.tar.gz appstore_production
...
Upload the **TechKiteLMSappstore.tar.gz** archive onto your production server.
...
.. code-block:: ini
    ...
    [uwsgi]
    chdir = /srv
    wsgi-file = /srv/TechKiteLMSappstore/wsgi.py
    master = true
    processes = 10
    vacuum = true
    uid = TechKiteLMSappstore
    gid = TechKiteLMSappstore
    socket = 0.0.0.0:8000
...
Then whitelist your container IP in **/etc/postgresql/9.5/main/pg_hba.conf**::
    ...
    host    TechKiteLMSappstore TechKiteLMSappstore 172.17.0.2/32       md5

.. note:: This expects the database user and database to be named **TechKiteLMSappstore**, your container IP to be **172.17.0.2** and host to run on **172.17.0.1**
...
and create a user and database::
    ...
    su - postgres
    psql
    CREATE USER TechKiteLMSappstore WITH PASSWORD 'password';
    CREATE DATABASE TechKiteLMSappstore OWNER TechKiteLMSappstore;
    \q
    exit
...
Then create a new configuration for it in **/etc/nginx/sites-available/TechKiteLMSappstore**:

.. code-block:: nginx

    upstream TechKiteLMSappstore {
    ...
    server {
        ...
        listen [::]:443 ssl http2;
        server_name apps.TechKiteLMS.com;
        charset     utf-8;

        # replace this with your ssl certificates
        ssl_certificate /etc/nginx/ssl/TechKiteLMSappstore.crt;
        ssl_certificate_key /etc/nginx/ssl/TechKiteLMSappstore.key;
        ...
        location / {
            uwsgi_pass TechKiteLMSappstore;
...
Finally replace your default configuration::
    ...
    sudo ln -s /etc/nginx/sites-available/TechKiteLMSappstore /etc/nginx/sites-enabled/default
...
Either create your own configuration or grab a copy of our `docker-compose.yml <https://github.com/TechKiteLMS/appstore/blob/master/docker-compose.yml>`_ and modify it if necessary. Place the file in your designated directory::

    cd /srv
    sudo wget https://raw.githubusercontent.com/TechKiteLMS/appstore/master/docker-compose.yml

Starting the Image
------------------
First load the latest uploaded image::

    sudo docker load -i /path/to/TechKiteLMSappstore.tar.gz
...
* **Application name**: TechKiteLMS App Store
* **Homepage URL**: https://apps.TechKiteLMS.com
* **Authorization callback URL**: https://apps.TechKiteLMS.com/github/login/callback/
...
Afterwards your **client id** and **client secret** are displayed. These need to be saved inside the database. To do that, either log into the admin interface, change your site's domain and add GitHub as a new social application or run the following command::
    ...
    sudo docker-compose exec python manage.py setupsocial --github-client-id "CLIENT_ID" --github-secret "SECRET" --domain apps.TechKiteLMS.com
...
Sync TechKiteLMS Releases from GitHub
-----------------------------------

The App Store needs to know about TechKiteLMS versions because:
...
You can run the command by giving it the oldest supported TechKiteLMS version::

     sudo docker-compose exec python manage.py syncTechKiteLMSreleases --oldest-supported="12.0.0"
...
You can also do a test run and see what kind of versions would be imported::
     ...
     sudo docker-compose exec python manage.py syncTechKiteLMSreleases --oldest-supported="12.0.0" --print
